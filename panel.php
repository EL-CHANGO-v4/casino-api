...
<div class="container">
    <div class="header">
        ...
        <div style="text-align: right;">
            ...
            <button onclick="window.location.reload()" class="btn-icon" style="font-size: 10px; margin-top: 5px;">🔄 REFRESCAR DATOS</button>
        </div>
    </div>
    ...
    <div class="card">
        <form method="POST" style="display: flex; gap: 10px;">
            ...
            <select name="r" style="background:#030712; border:1px solid var(--border); color:white; padding:8px; border-radius:8px;"><option value="JUGADOR">JUGADOR</option><option value="VENDEDOR">VENDEDOR</option><option value="ADMIN">ADMIN</option></select>
                ...
                <tr>
                    ...
                    <td>
                        <form method="POST" style="display: flex; gap: 5px;">
                            ...
                            <input type="number" name="monto" placeholder="0" style="width: 60px; background:#030712; border:1px solid var(--border); color:white; padding:5px; border-radius:6px;">
                            <button name="cargar_btn" class="btn-icon">↑</button>
                            <button name="vaciar_btn" class="btn-icon" style="color: #f59e0b;">↓</button>
                            <button name="borrar_btn" class="btn-icon" style="color: #ef4444;" onclick="return confirm('¿Borrar?')">🗑</button>
                        </form>
                    </td>
                </tr>
                <?php endwhile; ?>
    ...
    <div class="card">
        <div style="font-size: 11px; font-weight: 800; color: var(--primary); margin-bottom: 10px;">AUDITORÍA ADMINISTRATIVA</div>
        <table>
            <thead><tr><th>FECHA</th><th>USUARIO</th><th>ACCIÓN</th><th>MONTO</th><th>NUEVO SALDO</th></tr></thead>
            <tbody>
                <?php $rm = $conn->query($movs_q); while($m = $rm->fetch_assoc()): ?>
                <tr>
                    <td style="font-size: 11px; color: var(--text-muted);"><?= date('H:i:s', strtotime($m['fecha'])) ?></td>
                    <td><?= $m['usuario'] ?? 'N/A' ?></td>
                    <td><span class="tipo-<?= $m['tipo'] ?>" style="font-weight: 800;"><?= $m['tipo'] ?></span></td>
                    <td>$<?= number_format($m['monto'], 2) ?></td>
                    <td style="font-weight: 800;">$<?= number_format($m['saldo_nuevo'], 2) ?></td>
                </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
